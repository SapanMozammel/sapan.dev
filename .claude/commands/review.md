...
- `apollo-client` — load **only** when reviewing files that import from `src/lib/apollo/` (Apollo lands via `apollo-client-integration` PRD; rules apply only when Apollo is in use).
