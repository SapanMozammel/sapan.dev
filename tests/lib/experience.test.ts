...
describe('getRoleDuration', () => {
	...
	it('formats a multi-year, multi-month range (Nov 2020 → May 2024 = 3y 6m)', () => {
		expect(getRoleDuration('2020-11', '2024-05')).toBe('3y 6m');
	});

	it('omits the month part when the range is whole years', () => {
		expect(getRoleDuration('2020-01', '2023-01')).toBe('3y');
	});

	it('omits the year part when the range is under a year', () => {
		expect(getRoleDuration('2020-02', '2020-10')).toBe('8m');
	});
	...
	it('computes an open-ended role against the provided now', () => {
		expect(getRoleDuration('2024-06', undefined, new Date(2026, 3, 1))).toBe('1y 10m');
	});

	it('falls back to now when end is malformed', () => {
		expect(getRoleDuration('2024-06', 'not-a-date', new Date(2025, 5, 1))).toBe('1y');
	});
});
