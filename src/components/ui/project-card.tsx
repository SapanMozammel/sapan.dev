...
const ProjectCard = memo<ProjectCardProps>(({ project }) => {
	...
	return (
		<div className={cn('grid min-h-110 w-full grid-cols-1 overflow-hidden rounded-2xl border border-solid lg:grid-cols-2 lg:rounded-4xl backdrop-blur-3xl z-2', project?.className)}>
			...
			<div className='h-52 w-full px-4 pb-4 sm:h-80 sm:px-6 sm:pb-6 lg:h-full lg:pt-6'>
				<CursorTooltip content={TOOLTIP_CONTENT} className='shadow-info/10 dark:shadow-info/30 relative h-full w-full cursor-pointer overflow-hidden rounded-xl shadow-lg' onClick={handleClick}>
					<Image
						src={project.image}
