	...
	constructor() {
      ...
      `,
        ...
        void main() {
          ...
          // Dynamic color gradient between primary and success based on position + time
          float colorNoise = periodicNoise(vInitialPosition * 2.0, uTime * 0.3);
          float colorMix = colorNoise * 0.5 + 0.5;
          // Bias toward vWorldPosition.xz distance for spatial gradient
          colorMix = mix(colorMix, smoothstep(0.0, 8.0, distanceFromCenter), 0.4);
          vec3 particleColor = mix(uColor1, uColor2, colorMix);
          // Boost color saturation — push toward pure hue
          particleColor *= 1.3;
          // Sparkle peaks flash slightly brighter, keep color dominant
          particleColor = mix(particleColor, vec3(1.0), max(sparkleBrightness - 1.2, 0.0) * 0.3);

          float alpha = (1.2 - clamp(vDistance * 0.6, 0.0, 1.0))
      ...
      `,
			uniforms: {
				...
				uColor1: { value: new THREE.Color('#4a4ded') },
				uColor2: { value: new THREE.Color('#43ead4') },
			},
			transparent: true,
			depthWrite: false,
		});
